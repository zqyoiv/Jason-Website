#!/usr/bin/env node
/**
 * Build static HTML for each project from commision-project-data.js.
 ...
 * html/project/commission/[project-id].html.
 * Run from project root: node scripts/build-commission-pages.js
 */
...
const DATA_PATH = path.join(ROOT, 'js', 'commision-project-data.js');
const TEMPLATE_PATH = path.join(ROOT, 'html', 'project.html');
const OUT_DIR = path.join(ROOT, 'html', 'project', 'commission');
...
function loadCommissionProjects() {
  const code = fs.readFileSync(DATA_PATH, 'utf8');
  const context = { COMMISION_PROJECTS: undefined };
  vm.createContext(context);
  vm.runInContext(code, context);
  return context.COMMISION_PROJECTS || {};
}
...
function main() {
  ...
  const PROJECTS = loadCommissionProjects();
  ...
  console.log('Building ' + ids.length + ' project pages in html/project/commission/');
